...
export function writeWindsurfFormat(memories: NativeMemory[], targetPath: string): void {
  ...
  const projectContext = memories.find(m => 
  ...
  const userPrefs = memories.find(m => 
    m.title.toLowerCase().includes("preference") || 
  ...
  memories.forEach(memory => {
    if (memory !== projectContext && memory !== userPrefs) {
      const fileName = memory.title.toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '') + ".md";
...
export function writeCursorFormat(memories: NativeMemory[], targetPath: string): void {
  ...
  // Write user preferences
  const userPrefs = memories.filter(m => 
    m.title.toLowerCase().includes("preference") || 
    m.title.toLowerCase().includes("user")
  );
...
function parseWindsurfMarkdown(content: string, lastModified: string): NativeMemory | null {
  ...
  const lastContentIndex = contentLines.findIndex(line => 
    line.startsWith("---") || line.startsWith("*Last updated:")
  );
  
  const cleanContent = lastContentIndex >= 0 
